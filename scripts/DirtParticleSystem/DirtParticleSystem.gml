	...
	if (!_particleSystem)
	{
		...
		_particleSystem = new BBMOD_ParticleSystem(BBMOD_MODEL_PARTICLE, _material, 500)
			.add_modules(
				...
				new BBMOD_AddRealOverTimeModule(BBMOD_EParticle.HealthLeft, -1, 1),
				// Increase scale over time
				new BBMOD_AddVec3OverTimeModule(BBMOD_EParticle.ScaleX, new BBMOD_Vec3(2)),
				// Decrease alpha with health
				new BBMOD_MixRealFromHealthModule(BBMOD_EParticle.ColorA, 1, 0),
				// Apply gravity
